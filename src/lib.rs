...
pub fn run<'a, A:'a, B:'a, T1, T2, K1, K2, K3, A1, C1, C2, C3, C4>(
    list_a              : &mut T1,
    list_b              : &mut T2,
    key_comparer_a_b    : K1,
    key_comparer_a_a    : K2,
...
where
 T1 : Iterator<Item=&'a A>
,T2 : Iterator<Item=&'a B>
,K1 : Fn(&A, &B) -> Ordering
,K2 : Fn(&A, &A) -> Ordering
,K3 : Fn(&B, &B) -> Ordering
,A1 : Fn(&A, &B) -> bool
...

{
    ...
    loop {
        ...
        } else if a_opt.is_some() && b_opt.is_none() {
            only_exist_in_a(&a_opt.unwrap());
            last_a = a_opt;
            a_opt = list_a.next();
        } else if a_opt.is_none() && b_opt.is_some() {
